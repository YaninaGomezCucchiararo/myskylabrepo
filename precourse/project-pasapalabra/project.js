function pasapalabraGame() {
    let playerName = prompt('Bienvenido al juego de Pasapalabra! por favor, introduce tu nombre: ');
    let points = 0;
    let questions = [{
            letter: "a",
            answer: "abducir",
    ...
    while (!roscoCompleted()) {
        startRosco();
    }

    function showTotalPoints(){
        console.log ( 'Has completado el rosco con un total de ' + points + (' acierto/s'))
    }
    showTotalPoints();
    
    function roscoCompleted() {
    ...
    function startRosco() {
        for (i = 0; i < questions.length; i += 1) {
            if (questions[i].status === 0) {
                let answerPlayer = prompt(questions[i].question).toLowerCase();
                if (answerPlayer === 'pasapalabra') {
                    console.log('PASAPALABRA, continuamos con la siguiente pregunta:');
                } else if (answerPlayer === questions[i].answer) {
                    points += 1;
